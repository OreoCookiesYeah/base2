...
class Map:  # 地图类
    def __init__(self, img_path, x, y, window):
        self.img = pygame.image.load(img_path)  # 地图图片
        self.x = x  # 地图坐标
        self.y = y
        self.window = window  # 地图所在的窗口
...
def main():  # 一般将程序的入口定义为main函数
    ...
    enemy_list = []  # 记录每架敌机
    for _ in range(5):
        enemy_plane = EnemyPlane('res/img-plane_%d.png' % random.randint(1, 7), random.randint(0, WINDOW_W-100), random.randint(-500, -70), window)
